...
void ControllerClient::begin() {
  ...
  startWebServer();
  if (!provisioningMode_) { Serial.println("[CONTROLLER] Wi-Fi ready. Sending first heartbeat now..."); sendHeartbeat(); }
}

void ControllerClient::queueAck(const String& id, const char* status, const String& result) {
  if (id.isEmpty()) return;
  if (pendingAcks_.length()) pendingAcks_ += ',';
  pendingAcks_ += "{\"id\":\"" + id + "\",\"status\":\"" + String(status) + "\",\"result\":\"" + result + "\"}";
}
    ...
    if (type == "message") {
      ...
      if (tag.isEmpty()) { Serial.println("[OTA] Command missing payload.tag"); queueAck(id, "failed", "missing_tag"); }
        ...
        if (!context) { queueAck(id, "failed", "unable_to_allocate_ota_context"); }
          ...
          otaCommandId = id;
          otaRunning = true;
          const BaseType_t created = xTaskCreatePinnedToCore(otaTask, "ota_update", 12288, context, 1, NULL, 0);

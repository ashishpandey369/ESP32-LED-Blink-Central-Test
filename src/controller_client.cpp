...
void ControllerClient::startWebServer() {
  ...
  webServer.on("/info", HTTP_GET, [this]() {
    String body = "{\"deviceId\":\"" + deviceId_ + "\",\"firmwareVersion\":\"" + firmwareVersion_ + "\",\"buildId\":\"" + buildId_ + "\"}";
    webServer.send(200, "application/json", body);
  });
  webServer.on("/save", HTTP_POST, [this]() {
    if (!provisioningMode_) {
      webServer.send(403, "text/plain", "Provisioning mode is not active.");
      return;
    }
    const String ssid = webServer.arg("ssid");
    const String password = webServer.arg("password");
    if (ssid.isEmpty() || ssid.length() > 32 || password.length() > 63) {
      webServer.send(400, "text/plain", "Invalid Wi-Fi details.");
      return;
    }
    ...
    preferences.end();
    webServer.send(200, "text/html", "<h2>Saved.</h2><p>The ESP32 is restarting and will connect to Wi-Fi.</p>");
    delay(500);
    ESP.restart();
  });
...
void ControllerClient::handleProvisioningRequests() {
  ...
  const String html = String("<!doctype html><html><meta name='viewport' content='width=device-width,initial-scale=1'><title>ESP32 LED Blink</title><body style='font-family:system-ui;max-width:520px;margin:40px auto;padding:20px'><h1>ESP32 LED Blink</h1><p>Device: <b>") +
      htmlEscape(deviceId_) + "</b></p><p>Firmware: <b>" + htmlEscape(firmwareVersion_) + "</b></p><p>Device key is available in Serial Monitor.</p><form method='post' action='/save'><label>Wi-Fi</label><br><select name='ssid' style='width:100%;padding:12px'>" +
      options + "</select><br><br><label>Password</label><br><input name='password' type='password' style='width:100%;padding:12px'><br><button style='margin-top:16px;padding:12px;width:100%'>Connect</button></form><form method='get' action='/scan'><button style='margin-top:10px;padding:12px;width:100%'>Scan again</button></form></body></html>";
...
void ControllerClient::processCommands(const String& json) {
  ...
  while (true) {
    ...
    if (type == "message") {
      ...
      if (messagePos >= 0) {
        pendingMessage_ = jsonValue(json, "message", messagePos);
        Serial.printf("[COMMAND] Remote message: %s\n", pendingMessage_.c_str());
      }
    } else if (type == "ota") {
      const String tag = jsonValue(json, "tag", typePos);
      const String version = jsonValue(json, "version", typePos);
...
void ControllerClient::performOta(const String& tag, const String& version) {
  if (version == firmwareVersion_) {
    Serial.println("[OTA] Already running requested version.");
    return;
  }
  HTTPClient http;
...
bool ControllerClient::consumeMessage(String& message) {
  if (pendingMessage_.isEmpty()) return false;

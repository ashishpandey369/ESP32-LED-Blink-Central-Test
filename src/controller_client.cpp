...
void ControllerClient::loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // OTA owns the HTTPS connection while an update is running.
    // Keep the application/controller loop responsive, but do not open a
    // second heartbeat HTTPS connection that can contend for ESP32 memory.
